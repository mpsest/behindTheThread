...
class DirtyTalkController extends Controller
{
    ...
    {
        return response()->json(DirtyTalk::with(['keywords'])->latest()->limit(3)->get());
    }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $dirtyTalk = DirtyTalk::create($data);
        $keywordIds = [];
        foreach ($keywordNames as $keywordName) {
            $keyword = Keyword::firstOrCreate(['palavra' => $keywordName]);
            $keywordIds[] = $keyword->id;
        }
        $dirtyTalk->keywords()->sync($keywordIds);

        return response()->json($dirtyTalk->load('keywords'), 201);
    }
